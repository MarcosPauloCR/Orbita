// Phosphor — Originkit
// Fundo animado em WebGL (raymarching) que substituiu a lua desenhada em
// CSS. Usado através de <Backdrop />, que cuida do posicionamento e das
// cores do tema — evite usar este componente direto.
...
// STEPS é injetado no lugar de __STEPS__ antes de compilar: um loop de
// WebGL 1 precisa de limite constante, então a versão leve (celular) e a
// pesada (desktop) são dois programas compilados a partir da mesma fonte.
const FRAG_SRC = `
...
export function Phosphor(props: Props) {
  ...
  useEffect(() => {
    ...
    // Telas pequenas (celular) pagam caro por cada passo do raymarching —
    // menos passos deixa a névoa um pouco mais rala, mas evita esquentar o
    // aparelho e comer bateria num fundo puramente decorativo.
    const coarse =
      typeof window !== "undefined" && window.matchMedia("(pointer: coarse)").matches;
    const steps = coarse ? 40 : 80;

    const vs = compileShader(gl, gl.VERTEX_SHADER, VERT_SRC);
    const fs = compileShader(
      gl,
      gl.FRAGMENT_SHADER,
      FRAG_SRC.replace("__STEPS__", String(steps))
    );
    if (!vs || !fs) return;
