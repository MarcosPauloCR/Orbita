...
// Cores lidas do tema (--ink, --accent, --moon-a, --moon-b, --star) em vez
// do arco-íris original — combina com o dourado do resto do app e reage
// sozinho à troca claro/escuro, igual ao Backdrop fazia com o Phosphor.
const PALETTE_VARS = ["--ink", "--accent", "--moon-a", "--moon-b", "--star"] as const;
const FALLBACK_COLORS = ["#f3f1ea", "#d9c48a", "#f6eed6", "#d9c48a", "#565c6b"];

function readThemeColors(): string[] {
  const styles = getComputedStyle(document.documentElement);
  const colors = PALETTE_VARS.map((name, i) => styles.getPropertyValue(name).trim() || FALLBACK_COLORS[i]);
...
export const PixelStars = memo(
  () => {
    ...
    const animateCanvas = useCallback(
      (timestamp: number): void => {
        ...
        backgroundStarsRef.current.forEach((star) => {
          ...
          // Update twinkling (pausa sozinho se o usuário pediu menos animação)
          if (star.twinkle && !reduceMotionRef.current) {
    ...
    useEffect(() => {
      ...
      // Create shooting stars periodically (puladas se a pessoa pediu menos
      // animação — cadentes cruzando a tela toda são o oposto de sutil)
      ...
      // Recalcula a paleta quando o tema claro/escuro muda
...
PixelStars.displayName = "PixelStars";
